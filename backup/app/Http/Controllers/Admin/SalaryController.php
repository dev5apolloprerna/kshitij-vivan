...
class SalaryController extends Controller
{
    ...
    foreach ($months as $month) {
        // Skip Jan and Feb if it's the current year
        // if ($year == $currentYear && in_array($month, [1, 2])) {
        //     continue;
        // }
        ...
        if (!isset($existingEntries[$month])) {
            ...
            $missingMonths[] = [
                'month' => $month,
                'year' => $year,
                'sid' => $existingEntries[$month]->sid,
                'is_process' => $existingEntries[$month]->is_process,
                'confirm_date' => $existingEntries[$month]->confirm_date
            ];
        }
    }
    ...
    public function updateSalaryDetailData(Request $request)
    {
    	try
    	{
	        ...
	        }else
	        {
						...
						$balance_cl = $SalaryDetail->balance_cl+$SalaryDetail->balance_cf;
    ...
    public function updateAttendanceData(Request $request)
    {
	        ...
	        }else
	        {
	                	...
	                	$SalaryDetail->half_day_leave=$request->half_day ?? 0;
	                	$SalaryDetail->full_day_leave=$request->total_absent ?? 0;
	               	    $SalaryDetail->Total_A=$Total_A ?? 0;
	               	    $SalaryDetail->Total_B=$Total_B ?? 0;
	               	    $SalaryDetail->Leave_ded=$Leave_ded ?? 0;
